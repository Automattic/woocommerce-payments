...
class WC_REST_Payments_Survey_Controller_Test extends WP_UnitTestCase {
	...
	public function test_comments_only_request_is_accepted() {
		$this->http_client_mock
			->expects( $this->once() )
			->method( 'get_blog_id' )
			->willReturn( 123 );
		...
		$this->http_client_mock
			...
			->with(
				...
				$this->callback(
					function ( $argument ) {
						return 'wcpay-reports-feedback' === $argument['survey_id']
							&& '' === $argument['survey_responses']['rating']
							&& 'only text' === $argument['survey_responses']['comments']['text'];
					}
				)
			)
			->willReturn(
