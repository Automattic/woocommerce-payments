...
class WC_Payments_Token_Service_Test extends WP_UnitTestCase {
	...
	public function test_woocommerce_get_customer_payment_tokens_removes_unavailable_tokens() {
		...
		$another_gateway_token = WC_Helper_Token::create_token( 'another_gateway_token' );
		$another_gateway_token->set_gateway_id( 'other_gateway' );
		$another_gateway_token->save();

		$tokens = [ $token1, $unavailable_token, $another_gateway_token, $token2 ];
		...
		$this->assertEquals( 'another_gateway_token', $result_tokens[1]->get_token() );
	...
	public function test_woocommerce_get_customer_payment_tokens_migrates_old_tokens_live_account() {
		...
		$tokens = [ $old_token1, $old_token2, $another_gateway_token, $token ];
		...
		$this->assertFalse( $result_tokens[0]->meta_exists( '_wcpay_customer_id' ) );
		$this->assertEquals( 'cus_12345_test', $result_tokens[1]->get_meta( '_wcpay_customer_id' ) );
