 ...
 *
 * @covers \WCPay\Subscriptions\RecurringItemHelper
 */
class RecurringItemHelperTest extends WCPAY_UnitTestCase {
	...
	public function test_has_recurring_items_product_context_without_listener() {
		$product                                   = WC_Helper_Product::create_simple_product();
		WC_Subscriptions_Product::$is_subscription = true;
	...
	public function test_has_recurring_items_cart_subject_resolves_to_null_when_global_cart_unavailable() {
		...
		try {
			RecurringItemHelper::has_recurring_items( 'cart', 'not-a-cart' );
			$this->assertNull( $captured_subject );
		} finally {
