...
class ExperimentTest extends WCPAY_UnitTestCase {
	 ...
	 * capturing the anon_id it was constructed with.
	 *
	 * @param string      $abtest_variation  What the stubbed abtest returns.
	 ...
	 * @return Experiment
	 */
	private function build_experiment( string $abtest_variation, string $assignment_key, &$captured_anon_id = null ) {
		$mock_abtest = $this->createMock( Experimental_Abtest::class );
		$mock_abtest->method( 'get_variation' )->willReturn( $abtest_variation );

		return new class( $this->mock_legacy_proxy, $mock_abtest, $assignment_key, $captured_anon_id ) extends Experiment {
			/**
			...
			public function __construct( $legacy_proxy, $abtest, $key, &$captured ) {
			...
			protected function variations(): array {
				return [ self::VARIATION_CONTROL, 'treatment_a' ];
			}
	...
	public function test_returns_assigned_variation_and_passes_assignment_key() {
		$this->set_consent( true );
		$captured   = null;
		$experiment = $this->build_experiment( 'treatment_a', 'store_123', $captured );
	...
	public function test_returns_control_for_unknown_variation() {
		$this->set_consent( true );
		$captured   = null;
