...
class WC_Payments_Onboarding_Service_Test extends WCPAY_UnitTestCase {
	...
	public function test_set_test_mode_invalidates_eligibility_transient() {
		set_transient( WC_Payments_Test_To_Live_Banner::TRANSIENT_ELIGIBLE, '1', HOUR_IN_SECONDS );

		$this->onboarding_service->set_test_mode( true );

		$this->assertFalse(
			get_transient( WC_Payments_Test_To_Live_Banner::TRANSIENT_ELIGIBLE ),
		...
		set_transient( WC_Payments_Test_To_Live_Banner::TRANSIENT_ELIGIBLE, '1', HOUR_IN_SECONDS );
		$this->onboarding_service->set_test_mode( false );
		$this->assertFalse( get_transient( WC_Payments_Test_To_Live_Banner::TRANSIENT_ELIGIBLE ) );
	...
	public function test_maybe_handle_gateway_test_mode_toggle_no_op_when_unchanged() {
		...
		$this->assertSame( '1', get_transient( WC_Payments_Test_To_Live_Banner::TRANSIENT_ELIGIBLE ) );
		$this->assertFalse( get_option( WC_Payments_Onboarding_Service::TEST_MODE_ENABLED_DATE_OPTION ) );
