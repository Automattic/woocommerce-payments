...
test.describe( 'Multi-currency setup', { tag: '@critical' }, () => {
	...
	test( 'can disable the multi-currency feature', async ( { adminPage } ) => {
		await deactivateMulticurrency( adminPage );
	} );

	test( 'can enable the multi-currency feature', async ( { adminPage } ) => {
	...
	test.describe( 'Currency settings', () => {
		...
		test( 'can change the charm price manually', async ( {
			...
			await setCurrencyRate( adminPage, testData.currencyCode, '1.00' );
			await setCurrencyPriceRounding(
				adminPage,
				testData.currencyCode,
				'0'
