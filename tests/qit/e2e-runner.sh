...
# Build environment variable arguments for QIT as an array to avoid word-splitting
env_args=()
...
if [[ -n "${E2E_STRIPE_PUBLISHABLE_KEY:-}" ]]; then
    env_args+=( --env "E2E_STRIPE_PUBLISHABLE_KEY=${E2E_STRIPE_PUBLISHABLE_KEY}" )
fi
if [[ -n "${E2E_STRIPE_ACCOUNT_ID:-}" ]]; then
    env_args+=( --env "E2E_STRIPE_ACCOUNT_ID=${E2E_STRIPE_ACCOUNT_ID}" )
fi
if [[ -n "${E2E_STRIPE_LIVE_MODE:-}" ]]; then
    env_args+=( --env "E2E_STRIPE_LIVE_MODE=${E2E_STRIPE_LIVE_MODE}" )
fi

# Run our QIT E2E tests (qit.yml automatically loaded from current directory)
echo "Running QIT E2E tests..."
