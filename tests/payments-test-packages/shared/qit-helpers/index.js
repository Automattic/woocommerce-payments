...
async function makePostRequest( hostname, path, data ) {
	try {
		...
		const hasProtocol =
			finalHost.startsWith( 'http://' ) ||
			finalHost.startsWith( 'https://' );
...
async function getDomain() {
	...
	throw new Error(
		'No working domain found in QIT_SITE_URL, QIT_DOMAIN, QIT_INTERNAL_DOMAIN, or QIT_INTERNAL_NGINX.'
	);
}
...
const qit = {
	...
	async individualLogging( action ) {
		...
		try {
			const response = await makePostRequest(
				await getDomain(),
				'/wp-json/qit/v1/individual-log',
				{ qit_individual_log: action }
			);

			if (
				response.body &&
				typeof response.body.output !== 'undefined'
			) {
				return response.body.output;
			} else {
				console.error(
					'Invalid or missing "output" in response body:',
					response.body
				);
		...
		return new Promise( async ( resolve, reject ) => {
			...
			const processLine = ( line ) => {
				...
				const exitMatch = line.match(
					/__QIT_STREAM_EXIT__CODE__START__(\d+)__END/
				);
				if ( exitMatch ) {
			...
			try {
				...
				console.error(
					`[Node] Error starting streaming request: ${ outerErr.message }`
				);
				reject( outerErr );
			}
		} );
	...
	async exec( command, silent = false ) {
		...
		if (
			...
			typeof response.body.stdout === 'undefined' ||
			typeof response.body.stderr === 'undefined'
		) {
			console.error( command );
			console.error( response );
			throw new Error(
				'Invalid response: "status", "stdout" or "stderr" not found.'
			);
		}
		...
		if ( response.body.status !== 0 ) {
			...
			throw new Error(
				`Command failed with status ${ response.body.status }: ${ response.body.stdout } ${ response.body.stderr }`
			);
		}
	...
	async wp( command, silent = true ) {
		...
		const wpCommand = command.trim().startsWith( 'wp ' )
			? command
			: `wp ${ command }`;
	...
	setEnv( key, value ) {
		...
		try {
			...
			console.error(
				`Error writing to the environment file: ${ error.message }`
			);
			throw error;
		}
	},
	unsetEnv( key ) {
		...
		try {
			...
			console.error(
				`Error updating the environment file: ${ error.message }`
			);
			throw error;
		}
	},
