...
describe( 'Link with enabled split UPE', () => {
	...
	afterAll( async () => {
		await merchant.login();
		await merchantWCP.disablePaymentMethod( linkPaymentsCheckbox, false );
	...
	it( 'should use the saved Link account to fill in the details', async () => {
		...
		await expect( page ).toFill( '#billing_phone', billingAddress.phone );
		await expect( page ).toFill( '#billing_email', billingAddress.email );
