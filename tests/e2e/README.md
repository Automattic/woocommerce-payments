...
- **Express Checkout is partially covered here; the native wallet sheet is not.** ECE button rendering/gating and the tokenized-cart → order flow are covered by Playwright specs, which stub `window.Stripe` to fake only the Apple/Google Pay wallet sheet (the one part headless Playwright can't drive). The native sheet is manually verified on a real device/browser on a hosted test site, since real Apple/Google Pay sheets need environments headless Playwright can't simulate. So there is Express Checkout E2E coverage, but the real-device wallet confirmation stays a deliberate manual step.
