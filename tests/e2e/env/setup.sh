...
if [[ $FORCE_E2E_DEPS_SETUP || ! -d "$SERVER_PATH" ]]; then
	step "Fetching server (branch ${WCP_SERVER_BRANCH-add/minimum-amount-error})"
	...
	rm -rf "$SERVER_PATH"
	git clone --depth=1 --branch ${WCP_SERVER_BRANCH-add/minimum-amount-error} $WCP_SERVER_REPO "$SERVER_PATH"
else
