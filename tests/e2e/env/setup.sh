...
if [[ ! ${SKIP_WC_BLOCKS_TESTS} ]]; then
	...
	unzip -qq woo-gutenberg-products-block-$LATEST_RELEASE.zip

	echo "Moving the unzipped plugin files. This may require your admin password"
	sudo mv woo-gutenberg-products-block-$LATEST_RELEASE/* $E2E_ROOT/deps/woo-gutenberg-products-block

	cli wp plugin activate woo-gutenberg-products-block

	rm -rf woo-gutenberg-products-block-$LATEST_RELEASE
else
