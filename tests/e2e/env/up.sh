...
if [[ "$E2E_USE_LOCAL_SERVER" != false ]]; then
	...
	docker compose -f "$E2E_ROOT/deps/transact-platform-server-e2e/docker-compose.yml" -f "$E2E_ROOT/deps/transact-platform-server-e2e/docker-compose.e2e.yml" up -d
fi
