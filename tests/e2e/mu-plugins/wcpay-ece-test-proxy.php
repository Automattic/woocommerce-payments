 ...
 * Description: Fakes a Stripe Express Checkout wallet sheet so E2E tests can drive Google Pay / Apple Pay / Amazon Pay, which are otherwise browser/OS UI Playwright can't reach. Lives in tests/, so it only runs where it's deliberately copied into mu-plugins, and refuses to run on a live account.
 *
...
function wcpay_ece_test_proxy_inject() {
	...
	// Refuse to run on a live account - the only real foot-gun is copying this
	// into a live site's mu-plugins. is_test()/is_dev() throw before Mode
	// initializes, so treat any failure as "not test" and bail.
