...
- **Abilities API registrations** (`src/Internal/Abilities/AbilitiesRegistrar.php`): when you change the code path behind a registered ability (REST controller callback, backing Request class, capability gate), audit the ability registration for required updates (annotations, `input_schema`, description). Run `vendor/bin/phpunit --filter AbilitiesRegistrarTest` after such changes.
