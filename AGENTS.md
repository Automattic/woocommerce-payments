...
- **Abilities API drift:** when changing `WC_REST_Payments_Reports_Transactions_Controller`, `List_Transactions`, or other surfaces that back a registered ability in `src/Internal/Abilities/Abilities_Registrar.php`, check whether the ability still mirrors the surface (new filters, permission changes, response shape). The ability delegates to the REST controller via `rest_do_request()`, so behavior changes propagate; schema/`input_schema` changes do not — update the registrar.
