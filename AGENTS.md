...
- Migrations are one-shot: the `version_compare` threshold is the release that ships the migration, so a store that has updated past it never re-runs it. Editing a shipped migration's threshold re-runs it for every store crossing the new value - the codebase does this deliberately for idempotent cleanups (`Erase_Deprecated_Flags_And_Options`, `Payment_Method_Deprecation_Settings_Update`), and it is only safe because re-running those is a noop. A migration that transforms data is not idempotent: give it a new class and threshold instead of editing the shipped one.
- Every schema or data update must be rollback-safe: a rollback to the previous release must not fatal or corrupt data against the migrated state. Migrations do not need to be undoable - the delete-only cleanups satisfy this as-is. If old code cannot read the new format, the change needs a deprecation window, not a hard cutover.
