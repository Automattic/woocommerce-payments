...
class PaymentRequest {
	...
	public function get_payment_method(): PaymentMethodInterface {
		...
		if ( ! $is_woopayment_selected ) {
			throw new PaymentRequestException( __( 'WooPayments is not used during checkout.', 'woocommerce-payments' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Exception message, no escaping needed.
		}
		...
		if ( isset( $request[ $token_request_key ] ) && 'new' !== $request[ $token_request_key ] ) {
			...
			if ( is_null( $token ) ) {
				throw new PaymentRequestException( __( 'Invalid saved payment method (token) ID.', 'woocommerce-payments' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Exception message, no escaping needed.
			}
		...
		throw new PaymentRequestException( __( 'No valid payment method was selected.', 'woocommerce-payments' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Exception message, no escaping needed.
	}
