...
class WC_Payments_Admin {
	...
	public function __construct(
		...
		add_action( 'woocommerce_admin_order_data_after_payment_info', array( $this, 'render_order_edit_payment_details_container' ) );

		// Add menu items.
		add_action( 'admin_menu', array( $this, 'add_payments_menu' ), 0 );
		add_action( 'admin_init', array( $this, 'maybe_redirect_to_onboarding' ), 11 ); // Run this after the WC setup wizard and onboarding redirection logic.
		add_action( 'admin_enqueue_scripts', array( $this, 'maybe_redirect_overview_to_connect' ), 1 ); // Run this late (after `admin_init`) but before any scripts are actually enqueued.
		add_action( 'admin_enqueue_scripts', array( $this, 'register_payments_scripts' ) );
		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_payments_scripts' ), 12 );
		add_action( 'woocommerce_admin_field_payment_gateways', array( $this, 'payment_gateways_container' ) );
		add_action( 'woocommerce_admin_order_totals_after_total', array( $this, 'show_woopay_payment_method_name_admin' ) );
		add_action( 'woocommerce_admin_order_totals_after_total', array( $this, 'display_wcpay_transaction_fee' ) );

		$this->admin_child_pages = array(
			'wc-payments-overview'     => array(
			...
			'wc-payments-transactions' => array(
	...
	public function add_payments_menu() {
		...
		if ( $should_render_full_menu ) {
			if ( $this->account->is_card_present_eligible() && $this->account->has_card_readers_available() ) {
				$this->admin_child_pages['wc-payments-card-readers'] = array(
			...
			if ( $this->account->get_capital()['has_previous_loans'] ) {
				$this->admin_child_pages['wc-payments-capital'] = array(
			...
			if ( WC_Payments_Features::is_documents_section_enabled() ) {
				$this->admin_child_pages['wc-payments-documents'] = array(
			...
			$submenu[ $last_submenu_key ][] = array( // PHPCS:Ignore WordPress.WP.GlobalVariablesOverride.Prohibited
	...
	public function register_payments_scripts() {
		...
		WC_Payments::register_script_with_dependencies( 'WCPAY_ADMIN_ORDER_ACTIONS', 'dist/order', array( 'jquery-tiptip' ) );
			...
			if ( $order && WC_Payment_Gateway_WCPay::GATEWAY_ID === $order->get_payment_method() ) {
				...
				wp_localize_script(
					...
					array(
						...
						'formattedRefundAmount' => wp_strip_all_tags( wc_price( $refund_amount, array( 'currency' => $order->get_currency() ) ) ),
	...
	private function get_js_settings(): array {
		...
		if ( ! empty( $connect_incentive['id'] ) ) {
			$connect_url = add_query_arg( array( 'promo' => sanitize_text_field( $connect_incentive['id'] ) ), $connect_url );
		}

		$this->wcpay_js_settings = array(
			...
			'overviewTasksVisibility'       => array(
				'dismissedTodoTasks'     => get_option( 'woocommerce_dismissed_todo_tasks', array() ),
				'deletedTodoTasks'       => get_option( 'woocommerce_deleted_todo_tasks', array() ),
				'remindMeLaterTodoTasks' => get_option( 'woocommerce_remind_me_later_todo_tasks', array() ),
			),
	...
	public function display_wcpay_transaction_fee( $order_id ) {
		...
		<tr>
			...
			<td width="1%"></td>
			<td class="total">
				-<?php echo wp_kses( wc_price( $order->get_meta( '_wcpay_transaction_fee' ), array( 'currency' => $order->get_currency() ) ), 'post' ); ?>
			</td>
		</tr>
		<?php
	}
	...
	public function add_disputes_notification_badge() {
		...
		foreach ( $submenu[ self::PAYMENTS_SUBMENU_SLUG ] as $index => $menu_item ) {
			if ( 'wc-admin&path=/payments/disputes' === $menu_item[2] ) {
				...
				$submenu[ self::PAYMENTS_SUBMENU_SLUG ][ $index ][2] = admin_url( add_query_arg( array( 'filter' => 'awaiting_response' ), 'admin.php?page=' . $menu_item[2] ) ); // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
	...
	private function get_disputes_awaiting_response_count() {
		...
		$needs_response_statuses = array( 'needs_response', 'warning_needs_response' );
