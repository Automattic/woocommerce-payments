<?php
/**
 * Class Bancontact_Payment_Gateway
 *
...
/**
 * Bancontact Payment method extended from card payment method.
...
class Bancontact_Payment_Gateway extends WC_Payment_Gateway_WCPay {
	 ...
	 * @type string
	 */
	const GATEWAY_ID = 'woocommerce_payments_bancontact';

	const METHOD_ENABLED_KEY = 'bancontact_enabled';

	/**
	 * Bancontact Constrictor same parameters as WC_Payment_Gateway_WCPay constructor.
	 *
	...
	public function __construct( WC_Payments_API_Client $payments_api_client, WC_Payments_Account $account, WC_Payments_Customer_Service $customer_service, WC_Payments_Token_Service $token_service, WC_Payments_Action_Scheduler_Service $action_scheduler_service ) {
		...
		$this->method_title       = __( 'WooCommerce Payments - Bancontact', 'woocommerce-payments' );
		$this->method_description = __( 'Accept payments via Bancontact.', 'woocommerce-payments' );
		$this->title              = __( 'Bancontact', 'woocommerce-payments' );
		$this->description        = __( 'You will be redirected to Bancontact.', 'woocommerce-payments' );
	 ...
	 * This method is currently specific to the Bancontact redirect payment method.
	 *
		...
		try {
			...
			if ( 'succeeded' === $status ) {
				...
				if ( ! empty( $error ) ) {
					...
					Logger::log( sprintf( 'Bancontact failed: %s (%s)', $error_message, $error_code ) );
					/* translators: localized exception message */
					$order->update_status( 'failed', sprintf( __( 'Bancontact payment failed: %s', 'woocommerce-payments' ), $error_message ) );
				}

				wc_add_notice( __( 'Bancontact payment has failed. If you continue to see this notice, please contact the admin.', 'woocommerce-payments' ), 'error' );
			...
			/* translators: localized exception message */
			$order->update_status( 'failed', sprintf( __( 'Bancontact payment failed: %s', 'woocommerce-payments' ), $e->getLocalizedMessage() ) );
	...
	public function payment_fields() {
		try {
			wp_localize_script( 'wcpay-bancontact-checkout', 'wcpay_config', $this->get_payment_fields_js_config() );
			wp_enqueue_script( 'wcpay-bancontact-checkout' );
