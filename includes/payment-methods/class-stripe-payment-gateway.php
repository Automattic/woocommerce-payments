...
/**
 * Stripe Payment method extended from card payment method.
 * Loads different JS files and fields and handles a redirect payment method.
 */
class Stripe_Payment_Gateway extends WC_Payment_Gateway_WCPay {
	/**
	...
	/**
	 * Stripe Constrictor same parameters as WC_Payment_Gateway_WCPay constructor.
	 *
	...
	public function __construct( WC_Payments_API_Client $payments_api_client, WC_Payments_Account $account, WC_Payments_Customer_Service $customer_service, WC_Payments_Token_Service $token_service, WC_Payments_Action_Scheduler_Service $action_scheduler_service ) {
		...
		$this->title              = __( 'Stripe UPE', 'woocommerce-payments' );
		$this->description        = __( 'Please enter your payment details', 'woocommerce-payments' );

		add_action( 'wp', [ $this, 'maybe_process_redirect_order' ] );
		add_action( 'wp_ajax_create_upe_intent', [ $this, 'create_upe_intent_ajax' ] );
	}

	/**
	 * Ajax function for creating payment intent for UPE element
	 */
	public function create_upe_intent_ajax() {
		try {
			$is_nonce_valid = check_ajax_referer( 'wcpay_create_upe_intent_nonce', false, false );
			if ( ! $is_nonce_valid ) {
				throw new Exception(
			...
			$intent = $this->create_and_confirm_setup_intent( $payment_methods );
	...
	public function process_redirect_payment( $order_id ) {
		try {
			...
			if ( 'processing' === $status || 'succeeded' === $status ) {
				...
				if ( ! empty( $error ) ) {
					...
					Logger::log( sprintf( 'Stripe payment failed: %s (%s)', $error_message, $error_code ) );
					/* translators: localized exception message */
					$order->update_status( 'failed', sprintf( __( 'Stripe payment failed: %s', 'woocommerce-payments' ), $error_message ) );
				}

				wc_add_notice( __( 'Stripe payment has failed. If you continue to see this notice, please contact the admin.', 'woocommerce-payments' ), 'error' );
			...
			/* translators: localized exception message */
			$order->update_status( 'failed', sprintf( __( 'Stripe payment failed: %s', 'woocommerce-payments' ), $e->getLocalizedMessage() ) );
	...
	/**
	 * Renders the Stripe input fields needed to get the user's payment information on the checkout page.
	 *
			...
			<?php if ( $this->is_in_test_mode() ) : ?>
				<p class="testmode-info">
					...
					/* translators: link to Stripe testing page */
						__( '<strong>Test mode:</strong> You may be redirected to a test Stripe page to approve or deny payment.', 'woocommerce-payments' ),
						[
							'strong' => '<strong>',
						]
					);
	...
	public function process_payment( $order_id ) {
		...
		try {
			...
			$intent_api_parameters = [
				'payment_method_types' => [ 'card', 'sofort', 'giropay', 'sepa' ],
