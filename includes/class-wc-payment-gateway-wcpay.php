...
<?php else : ?>
	...
	public function capture_charge( $order, $include_level3 = true, $intent_metadata = [] ) {
		...
		try {
			...
			// Skip level3 for Express Checkout payments (Apple Pay, Google Pay, Amazon Pay)
			// as multi-type payment intents (e.g. card + amazon_pay) don't support level3.
			if ( $include_level3 && ! $order->get_meta( '_wcpay_express_checkout_payment_method' ) ) {
