...
class WC_Payment_Gateway_WCPay extends WC_Payment_Gateway_CC {
	...
	protected function prepare_payment_information( $order ) {
		...
		if ( ! empty( $_POST['save_user_in_platform_checkout'] ) && 'true' === $_POST['save_user_in_platform_checkout'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	protected function get_metadata_from_order( $order, $payment_type ) {
		...
		// TODO: Remove this workaround when `get_billing_email` starts working again. See #3662.
		if ( empty( $email ) ) {
