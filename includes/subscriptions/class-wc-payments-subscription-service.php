...
class WC_Payments_Subscription_Service {
	...
	public function __construct(
		...
		add_action( 'woocommerce_subscription_payment_method_updated_from_' . WC_Payment_Gateway_WCPay::get_card_gateway_id(), [ $this, 'maybe_cancel_subscription' ], 10, 2 );
	}
	...
	public static function is_wcpay_subscription( WC_Subscription $subscription ): bool {
		return ! WC_Payments_Subscriptions::is_duplicate_site() && WC_Payment_Gateway_WCPay::get_card_gateway_id() === $subscription->get_payment_method() && (bool) self::get_wcpay_subscription_id( $subscription );
	}
	...
	public function create_subscription( WC_Subscription $subscription ) {
		 ...
		 */
		if ( WC_Payment_Gateway_WCPay::get_card_gateway_id() !== $subscription->get_payment_method() ) {
			return;
		}
	...
	public function maybe_cancel_subscription( $subscription, $new_payment_method ) {
		...
		if ( (bool) $wcpay_subscription_id && WC_Payment_Gateway_WCPay::get_card_gateway_id() !== $new_payment_method ) {
