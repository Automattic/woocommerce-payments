...
( function ( a ) {
	...
	function W( b ) {
		...
		var c,
			...
			j = 0;
		for (
			e = O( b._f, b._locale ).match( Nb ) || [], c = 0;
			c < e.length;
			c++
		)
			( f = e[ c ] ),
	...
	function _( a ) {
		$( a ),
			a._isValid === ! 1 &&
				( delete a._isValid, tb.createFromInputFallback( a ) );
	}
	function ab( a, b ) {
		var c,
			d = [];
	...
	for (
		var tb,
			...
			Nb = /(\[[^\[]*\])|(\\)?(Mo|MM?M?M?|Do|DDDo|DD?D?D?|ddd?d?|do?|w[o|w]?|W[o|W]?|Q|YYYYYY|YYYYY|YYYY|YY|gg(ggg?)?|GG(GGG?)?|e|E|a|A|hh?|HH?|mm?|ss?|S{1,4}|x|X|zz?|ZZ?|.)/g,
	...
	( pc.DDDD = h( pc.DDD, 3 ) ),
		...
		( tb.isMoment = function ( a ) {
			return (
				a instanceof k || ( null != a && c( a, '_isAMomentObject' ) )
			);
		} ),
	...
	( tb.normalizeUnits = function ( a ) {
		...
		( tb.invalid = function ( a ) {
			var b = tb.utc( 0 / 0 );
			return (
				null != a ? m( b._pf, a ) : ( b._pf.userInvalidated = ! 0 ), b
			);
		} ),
		...
		m( ( tb.fn = k.prototype ), {
			...
			diff: function ( a, b, c ) {
				...
				return (
					...
					'year' === b || 'month' === b
						? ( ( d =
								432e5 *
								( this.daysInMonth() + g.daysInMonth() ) ),
						  ( e =
								12 * ( this.year() - g.year() ) +
								( this.month() - g.month() ) ),
						  ( f =
				...
				var b = a || tb(),
					...
					e =
						-6 > d
							...
							: 'sameElse';
				return this.format(
					this.localeData().calendar( e, this, tb( b ) )
				);
			},
			isLeapYear: function () {
				return E( this.year() );
			...
			min: f(
				...
				function ( a ) {
					return (
						( a = tb.apply( null, arguments ) ), this > a ? this : a
					);
				}
			),
			max: f(
				...
				function ( a ) {
					return (
						( a = tb.apply( null, arguments ) ), a > this ? this : a
					);
				}
			),
			zone: function ( a, b ) {
		...
		m( ( tb.duration.fn = l.prototype ), {
			...
			as: function ( a ) {
				...
				switch (
					( ( b =
						this._days + Math.round( qb( this._months / 12 ) ) ),
					a )
				) {
					case 'week':
