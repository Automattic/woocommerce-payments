    ...
    ************************************/
	...
	function addOrSubtractDurationFromMoment(
		...
		if ( days ) {
			rawSetter(
				mom,
				'Date',
				rawGetter( mom, 'Date' ) + days * isAdding
			);
		}
		if ( months ) {
			rawMonthSetter(
				mom,
				rawGetter( mom, 'Month' ) + months * isAdding
			);
		}
		if ( updateOffset ) {
	...
	function normalizeUnits( units ) {
		if ( units ) {
			...
			units =
				unitAliases[ units ] || camelFunctions[ lowered ] || lowered;
		}
		return units;
	}
	...
	function weeksInYear( year, dow, doy ) {
		return weekOfYear( moment( [ year, 11, 31 + dow - doy ] ), dow, doy )
			.week;
	}
			...
			if ( units === 'year' || units === 'month' ) {
				...
				daysAdjust -=
					( this.zone() -
						...
						( that.zone() -
							moment( that ).startOf( 'month' ).zone() ) ) *
					6e4;
	...
	extend( ( moment.duration.fn = Duration.prototype ), {
		...
		as: function ( units ) {
			...
			if ( units === 'month' || units === 'year' ) {
				...
				days =
					this._days + Math.round( yearsToDays( this._months / 12 ) );
				switch ( units ) {
					case 'week':
