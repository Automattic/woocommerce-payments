		...
		var HEMISPHERE_SOUTH = 's',
			...
			lookup_key = function lookup_key() {
				var january_offset = get_date_offset(
					...
					june_offset = get_date_offset(
						new Date( consts.BASELINE_YEAR, 5, 2 )
					),
			 ...
			 * @returns {Date}
			 */
			find_dst_fold = function find_dst_fold(
				a_date,
				padding,
				iterator
			) {
					...
					for ( var j = 0; j < rule_list.length; j++ ) {
						...
						if ( !! sample.rules[ j ] && !! rule_list[ j ] ) {
							...
							if (
								...
								score += Math.abs(
									sample.rules[ j ].e - rule_list[ j ].e
								);
			...
			get_by_dst = function get_by_dst( preliminary_timezone ) {
				var get_rules = function get_rules() {
					...
					for (
						var i = 0;
						i < jstz.olson.dst_rules.years.length;
						i++
					) {
						var year_rules = dst_dates(
							jstz.olson.dst_rules.years[ i ]
						);
			...
			determine = function determine() {
				...
				if ( ! preliminary_tz ) {
					...
					if (
						typeof consts.AMBIGUITIES[ preliminary_tz ] !==
						'undefined'
					) {
