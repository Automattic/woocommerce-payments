...
final class Intention extends Data_Transfer_Object implements \JsonSerializable {
	/**
	 * Intention ID
	 *
	 * @var string
	 ...
	 * @param Charge|null            $charge             - An array containing payment method details of associated charge.
	...
	public static function create_from_wc_pay_response(Response $response) {
		...
		$charge_array       = 0 < $data['charges']['total_count'] ? end( $data['charges']['data'] ) : null;
		$next_action        = ! empty( $data['next_action'] ) ? $data['next_action'] : [];
		$last_payment_error = ! empty( $data['last_payment_error'] ) ? $data['last_payment_error'] : [];
		$metadata           = ! empty( $data['metadata'] ) ? $data['metadata'] : [];
		$customer           = $data['customer'] ?? $charge_array['customer'] ?? null;
		$payment_method     = $data['payment_method'] ?? $data['source'] ?? null;

		if ( ! empty ($charge_array) ) {
			$charge = Create_Charge_Dto::create_from_wc_pay_response($charge_array);
		}
