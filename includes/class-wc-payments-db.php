...
class WC_Payments_DB {
	...
	public function order_from_intent_id( $intent_id ) {
		$order_id = $this->order_id_from_intent_id( $intent_id );
	...
	/**
	 * Retrieve an order ID from the DB using a corresponding Stripe intent ID.
	 *
	 * @param string $intent_id Intent ID corresponding to an order ID.
	 *
	 * @return null|string
	 */
	private function order_id_from_intent_id( $intent_id ) {
		global $wpdb;

		// The order ID is saved to DB in `WC_Payment_Gateway_WCPay::process_payment()`.
		$order_id = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT DISTINCT ID FROM $wpdb->posts as posts LEFT JOIN $wpdb->postmeta as meta ON posts.ID = meta.post_id WHERE meta.meta_value = %s AND meta.meta_key = '_intent_id'",
				$intent_id
			)
		);
		return $order_id;
