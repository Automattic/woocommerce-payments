...
class WC_Payments {
	...
	public static function get_combined_translation_filename( $locale ) {
		return implode( '-', [ 'woocommerce-payments', $locale ] ) . '.json';
	}
	...
	public static function combine_translation_chunk_files( $instance, $hook_extra ) {
		...
		foreach ( $hook_extra['translations'] as $translation ) {
			if (
				...
				'woocommerce-payments' === $translation['slug']
			) {
	...
	public static function get_translation_chunk_data( $locale ) {
		...
		$json_i18n_filenames       = glob( WP_LANG_DIR . '/plugins/woocommerce-payments-' . $locale . '-*.json' );
