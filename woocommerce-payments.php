...
function wcpay_jetpack_init() {
	...
	$jetpack_config->ensure(
		...
		[
			...
			'name' => __( 'WooPayments', 'woocommerce-payments' ),
		]
	);
	$jetpack_config->ensure(
		'identity_crisis',
...
function wcpay_show_old_jetpack_notice() {
	...
	<div class="notice wcpay-notice notice-error">
		...
		<p><?php echo esc_html( __( 'The version of Jetpack installed is too old to be used with WooPayments. WooPayments has been disabled. Please deactivate or update Jetpack.', 'woocommerce-payments' ) ); ?></p>
	</div>
	<?php
}
...
function wcpay_get_jetpack_idc_custom_content(): array {
	$custom_content = [
		...
		'mainBodyText'              => __( 'We’ve detected that you have duplicate sites connected to WooPayments. When Safe Mode is active, payments will not be interrupted. However, some features may not be available until you’ve resolved this issue below. Safe Mode is most frequently activated when you’re transferring your site from one domain to another, or creating a staging site for testing. <safeModeLink>Learn more</safeModeLink>', 'woocommerce-payments' ),
		'migratedTitle'             => __( 'WooPayments connection successfully transferred', 'woocommerce-payments' ),
		'migratedBodyText'          => __( 'Safe Mode has been deactivated and WooPayments is fully functional.', 'woocommerce-payments' ),
		...
		'nonAdminBodyText'          => __( 'We’ve detected that you have duplicate sites connected to WooPayments. When Safe Mode is active, payments will not be interrupted. However, some features may not be available until you’ve resolved this issue below. Safe Mode is most frequently activated when you’re transferring your site from one domain to another, or creating a staging site for testing. A site adminstrator can resolve this issue. <safeModeLink>Learn more</safeModeLink>', 'woocommerce-payments' ),
		'supportURL'                => 'https://woocommerce.com/document/payments/faq/safe-mode/',
		'adminBarSafeModeLabel'     => __( 'WooPayments Safe Mode', 'woocommerce-payments' ),
	...
	if ( false !== $urls ) {
		...
		$custom_content['migrateCardBodyText'] = sprintf(
			...
			__(
				'Transfer your WooPayments connection from <hostname>%2$s</hostname> to this site <hostname>%1$s</hostname>. <hostname>%2$s</hostname> will be disconnected from WooPayments.',
		...
		$custom_content['startFreshCardBodyText'] = sprintf(
			/* translators: %1$s: The current site domain name. %2$s: The original site domain name. Please keep hostname tags in your translation so that they can be formatted properly. */
			__(
				'Create a new connection to WooPayments for <hostname>%1$s</hostname>. You’ll have to re-verify your business details to begin accepting payments. Your <hostname>%2$s</hostname> connection will remain as is.',
				'woocommerce-payments'
			),
			$current_url,
			$wpcom_url
