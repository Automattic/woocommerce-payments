...
if ( ! $order || 'yes' !== $order->get_meta( '_wcpay_agent_purchase' ) || ! $order->is_paid() || ! $order->get_transaction_id() || 'test' !== $order->get_meta( '_wcpay_mode' ) ) { throw new RuntimeException( 'Expected a paid demo test order.' ); }
...
$original = get_option( 'wcpay_agent_purchases_web_enabled', 'yes' );
try {
	...
	if ( isset( $gateways['woocommerce_payments'] ) || ! isset( $gateways['bacs'] ) || 'yes' !== get_option( 'wcpay_agent_purchases_enabled' ) ) { throw new RuntimeException( 'Independent channel settings failed.' ); }
	update_option( 'wcpay_agent_purchases_web_enabled', 'yes' );
...
} finally { update_option( 'wcpay_agent_purchases_web_enabled', $original ); }
