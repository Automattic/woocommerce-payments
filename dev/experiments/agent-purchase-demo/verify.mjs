...
const root = `${config.baseUrl}/?rest_route=/wcpay-agent-demo/v1`;
async function request(path, { auth = true, method = 'GET', body } = {}) {
  const response = await fetch(`${root}${path}`, {
    method,
    headers: { 'Content-Type': 'application/json', ...(auth ? { 'X-WCPay-Agent-Demo': config.key } : {}) },
...
const quoted = await request('/quotes', { method: 'POST', body: { product_id: products.body.products[0].id, quantity: 1 } });
...
console.log(JSON.stringify({ checks: 11, quote_id: quoted.body.id, approval_url: quoted.body.approval_url, total: quoted.body.quote.total }, null, 2));
