...
| `type` string | `type_is` |
| `type` array or comma-delimited list | `type_is_in` |
...
Phase 0 adds these accepted params for the later `available_on` date basis:

- `available_on_after`
- `available_on_before`

For Phase 1, `type` accepts a single value or a list. A single value maps to `type_is`; a list maps to `type_is_in`.

## Row Contract

The Fees list reuses `WC_REST_Payments_Reports_Transactions_Controller::prepare_item_for_response()` and returns report-shaped rows, not raw transaction-cache rows.
...
The Method filter uses `sources` from this response. The Type filter uses a `types` array if the server provides one; otherwise it uses the current report type label set as the MVP fallback.
...
After Phase 0 deploys `available_on_after` and `available_on_before` support on the transactions list, summary, and download endpoints, the Fees report can switch the period filter to `available_on_*` and remove the footnote.

## Implementation Notes

- The list route uses the typed `List_Transactions` request.
- Summary and export route callbacks use `forward_request()` to call Fees-specific API-client wrappers that delegate to the transactions summary/download endpoints. This is a deliberate legacy-pattern exception isolated to summary/export.
- Tests assert converted transaction filter keys (`source_is`, `type_is`/`type_is_in`, `order_id_is`, `customer_email_is`) rather than raw report params.
