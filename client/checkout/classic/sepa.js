...
jQuery( function ( $ ) {
	...
	$( document.body ).on( 'updated_checkout', () => {
		...
		if (
			! $( '#wcpay-sepa-card-element' ).length ||
			$( '#wcpay-sepa-card-element' ).children().length
		) {
            console.log("WHY is it not finding it");
			return;
		}

		cardElement.unmount();
		cardElement.mount( '#wcpay-sepa-card-element' );
	} );
	...
	$( '#order_review' ).on( 'submit', () => {
		if (
			$( '#payment_method_woocommerce_payments_sepa' ).is( ':checked' ) &&
