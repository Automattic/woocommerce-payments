...
export const filters = 	[
	{
		...
		filters: [
			...
			{ label: __( 'Advanced filters', 'woocommerce-payments' ), value: 'advanced' },
		],
	},
];
...
export const advancedFilters = {
	...
	title: __( 'Transactions match {{select /}} filters', 'woocommerce-payments' ),
	filters: {
		date: {
			labels: {
				add: __( 'Date', 'woocommerce-payments' ),
				remove: __( 'Remove transaction date filter', 'woocommerce-payments' ),
				rule: __( 'Select a transaction date filter match', 'woocommerce-payments' ),
				/* translators: A sentence describing a Transaction date filter. See screen shot for context: https://d.pr/i/NcGpwL */
				title: __( '{{title}}Date{{/title}} {{rule /}} {{filter /}}', 'woocommerce-payments' ),
				filter: __( 'Select a transaction date', 'woocommerce-payments' ),
			},
            rules: [
                {
                    value: 'before',
		...
		type: {
			labels: {
				...
				remove: __( 'Remove transaction type filter', 'woocommerce-payments' ),
				rule: __( 'Select a transaction type filter match', 'woocommerce-payments' ),
				/* translators: A sentence describing a Transaction type filter. See screen shot for context: https://d.pr/i/NcGpwL */
				title: __( '{{title}}Type{{/title}} {{rule /}} {{filter /}}', 'woocommerce-payments' ),
				filter: __( 'Select a transaction type', 'woocommerce-payments' ),
			},
			rules: [
				{
					value: 'is',
					...
					label: _x( 'Is', 'transaction type', 'woocommerce-payments' ),
				},
				{
					value: 'is_not',
					...
					label: _x( 'Is not', 'transaction type', 'woocommerce-payments' ),
				},
			],
			input: {
