...
function make(idOpt, object_Opt, amountOpt, amount_refundedOpt, applicationOpt, application_feeOpt, application_fee_amountOpt, balance_transactionOpt, billing_detailsOpt, calculated_statement_descriptorOpt, capturedOpt, createdOpt, currencyOpt, disputeOpt, disputedOpt, level3Opt, livemodeOpt, orderOpt, outcomeOpt, paidOpt, payment_intentOpt, payment_methodOpt, payment_method_detailsOpt, receipt_emailOpt, receipt_numberOpt, receipt_urlOpt, refundedOpt, refundsOpt, statusOpt, param) {
  ...
  var created = createdOpt !== undefined ? createdOpt : 0.0;
...
function getAmounts(charge) {
  var isChargeRefunded = charge.amount_refunded > 0;
  var isChargeDisputed = charge.disputed;
  var isDisputeWon = Belt_Option.getWithDefault(Belt_Option.map(charge.dispute, (function (d) {
              return d.status !== /* Won */6;
            })), false);
  var fee = isChargeDisputed && isDisputeWon ? Belt_Option.getWithDefault(charge.application_fee_amount, 0) + 1500 | 0 : Belt_Option.getWithDefault(charge.application_fee_amount, 0);
  var match = isChargeDisputed && isDisputeWon;
  var refunded = match ? Belt_Option.getWithDefault(Belt_Option.map(charge.dispute, (function (d) {
                return d.amount;
              })), 0) : (
      isChargeRefunded ? charge.amount_refunded : 0
    );
  var net = (charge.amount - fee | 0) - refunded | 0;
  return {
          net: net,
          fee: fee,
          refunded: refunded
