 ...
 * @return {import('wcpay/types/wcpay-data-settings-hooks').GenericSettingsHook<string[]>}
 */
export const useExpressCheckoutProductMethods = () => {
	const { updateExpressCheckoutProductMethods } = useDispatch( STORE_NAME );

	const expressCheckoutProductMethods = useSelect( ( select ) =>
		select( STORE_NAME ).getExpressCheckoutProductMethods()
	);

	return [
		expressCheckoutProductMethods,
		updateExpressCheckoutProductMethods,
	];
};
 ...
 * @return {import('wcpay/types/wcpay-data-settings-hooks').GenericSettingsHook<string[]>}
 */
export const useExpressCheckoutCartMethods = () => {
	const { updateExpressCheckoutCartMethods } = useDispatch( STORE_NAME );

	const expressCheckoutCartMethods = useSelect( ( select ) =>
		select( STORE_NAME ).getExpressCheckoutCartMethods()
	);

	return [ expressCheckoutCartMethods, updateExpressCheckoutCartMethods ];
};

/**
 * @return {import('wcpay/types/wcpay-data-settings-hooks').GenericSettingsHook<string[]>}
 */
export const useExpressCheckoutCheckoutMethods = () => {
	const { updateExpressCheckoutCheckoutMethods } = useDispatch( STORE_NAME );

	const expressCheckoutCheckoutMethods = useSelect( ( select ) =>
		select( STORE_NAME ).getExpressCheckoutCheckoutMethods()
	);

	return [
		expressCheckoutCheckoutMethods,
		updateExpressCheckoutCheckoutMethods,
	];
};
