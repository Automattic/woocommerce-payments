...
describe( 'Express checkout event handlers', () => {
	...
	describe( 'onConfirmHandler', () => {
		...
		it( 'should abort payment if shipping rate is pending', async () => {
			const eventWithPendingRate = {
				...event,
		...
		it( 'should allow payment if shipping rate is not pending', async () => {
			cartApiPlaceOrderMock.mockResolvedValue( {
				order_id: 123,
				status: 'processing',
				order_key: 'wc_order_test123',
		...
		it( 'should allow payment when shippingRate is undefined (virtual products)', async () => {
