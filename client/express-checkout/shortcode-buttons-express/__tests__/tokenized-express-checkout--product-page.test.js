...
describe( 'Tokenized Express Checkout Element - Product page logic', () => {
	...
	it( 'hides the express button on ready when add to cart is blocked', async () => {
		document
		...
		// triggering the `ready` event with available methods.
		stripeElementMock.__getRegisteredEvent( 'ready' )( {
			availablePaymentMethods: {
				link: false,
				applePay: true,
	...
	it( 'hides the express button when the selection becomes blocked', async () => {
		apiFetch.mockImplementation( async () =>
			Promise.resolve( {
				json: () => Promise.resolve( { items: [] } ),
				headers: new Map(),
		...
		// becomes visible on ready with available methods.
		stripeElementMock.__getRegisteredEvent( 'ready' )( {
			availablePaymentMethods: {
				link: false,
				applePay: true,
		...
		// shopper picks an unavailable variation: add-to-cart becomes disabled.
		document
			.querySelector( '.single_add_to_cart_button' )
			.classList.add( 'disabled' );
